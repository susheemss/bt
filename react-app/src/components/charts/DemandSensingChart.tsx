...
export default function DemandSensingChart({ labels, forecast, sensed }: Props) {
  ...
  return (
    <ResponsiveContainer width="100%" height={172}>
      <ComposedChart data={data} margin={{ top: 8, right: 4, left: -14, bottom: 0 }}>
        ...
        <Line type="monotone" dataKey="baseline" name="baseline" stroke="#8B95A5" strokeDasharray="5 3" strokeWidth={1.5} dot={false} />
        <Line type="monotone" dataKey="sensed" name="sensed" stroke="#6C5CE7" strokeWidth={2.2}
              dot={{ r: 2.5, fill: '#6C5CE7', strokeWidth: 0 }} activeDot={{ r: 4 }} />
      </ComposedChart>
    </ResponsiveContainer>
  )
}
